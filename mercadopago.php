...
require PLUGINS . 'jpay/vendor/autoload.php';
        ...
        if (isset($id)) {
            ...
            if ($status == 'approved') {
                ...
                $coins = $userCoins + ($refCoins * $multiplier);
								// $coinsWithBonus = JLib::getDonateBonus($coins, true);
								$coin_type = $config['mercadopago']['COIN_TYPE'];
								
								// Jlib::addBackupCoinsByEmail($user['email'], $coinsWithBonus);
								
                $db->query("UPDATE `accounts` SET `{$coin_type}` = '{$coins}' WHERE `id` = '{$user['id']}'");
				
				$new_bkp_coins = $user['backup_coins'] + ($refCoins * $multiplier);
				
                $db->query("UPDATE `accounts` SET `backup_coins` = '{$new_bkp_coins}' WHERE `id` = '{$user['id']}'");
