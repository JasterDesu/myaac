              ...
              <tbody>
				...
				<tr>
                  ...
                  <td><select name="category">
                      ...
                      <option value="fist">Atk Speed</option>
...
<?php
	...
	if(empty($rank_vocation)){
		if(isset($rank_category) && !empty($rank_category)){
			header('Location: '. BASE_URL . ($config['friendly_urls'] ? '' : '?') .'highscores/'.$rank_category.'');
		}
	}else{
		if(isset($rank_category)){
			header('Location:  '. BASE_URL .($config['friendly_urls'] ? '' : '?') .'highscores/'.$rank_category.'/'.$rank_vocation.'');
		}
	}
?>
                      ...
                      <table class="TableContent" width="100%" style="border:1px solid #faf0d7;">
                        <tbody>
                          <tr class="LabelH">
                            ...
                            <td>Name</td>
                            <td>Vocation</td>
                            <td><?php echo ($skill != SKILL_FRAGS && $skill != SKILL_BALANCE ? 'Level' : ($skill == SKILL_BALANCE ? 'Balance' : 'Frags')); ?></td>
...
?>
                ...
                <tr>
                  <td>
                    ...
                    <?php
                      if($_page > 0){
                        echo '<A HREF="' . getLink('highscores') . '/' . $list . (isset($vocation) ? '/' . $vocation : '') . '/' . ($_page - 1) . ' " CLASS="size_xxs">Previous Page</A>  | ';
                      }
                      if($show_link_to_next_page){
                        echo '<A HREF="' . getLink('highscores') . '/' . $list . (isset($vocation) ? '/' . $vocation : '') . '/' . ($_page + 1) . ' " CLASS="size_xxs">Next Page</A>';
                      }
                    ?>
                    </center>
                  </td>
